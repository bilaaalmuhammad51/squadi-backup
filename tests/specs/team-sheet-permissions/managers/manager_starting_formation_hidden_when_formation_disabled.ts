...
describe("Starting Formation row is hidden for Manager when the formation flag is disabled (S6)", () => {
  before(async () => {
    await step("Create match, then disable gameTimeTracking on the competition", async () => {
      ...
      await MatchApiHelper.updateCompetitionScoring(token, "COURT", undefined, {
  ...
  it("should not show the Starting Formation row in Game Details even though Manager normally has field_positions", async () => {
    ...
    await step(
      "Validate Team Sheet is present but Starting Formation is not",
      async () => {
        await scorerPage.waitUntilVisibleWithRetry(scorerPage.teamSheetOption);
        await scorerPage.assertElementDisplayed(scorerPage.fieldOption);
        await scorerPage.assertElementDisplayed(scorerPage.teamSheetOption);

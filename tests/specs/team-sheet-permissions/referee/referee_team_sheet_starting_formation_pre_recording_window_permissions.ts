...
describe("Referee team sheet pre-recording window permissions", () => {
  it("log in as Referee, confirm the Substitution row is never shown (S10), and submit the Team Sheet for both teams before the recording window opens", async () => {
    ...
    await step("Create match before launching app", async () => {
      matchId = await MatchApiHelper.createAndPublishMatch(15);

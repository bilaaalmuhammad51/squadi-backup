...
describe("Match Scoring Flow", () => {
    it("should log in with valid credentials, open a match, manage team sheets, start or resume play, and validate score increment and undo actions", async () => {
        ...
        await step("Handle team sheet flow if alert is displayed", async () => {
            ...
            if (isVisible) {
                ...
                await step("Validate and submit home team players", async () => {
                    ...
                    await scorerPage.submitHomeTeamPlayersIfNotSubmitted();
                });
        ...
        await step("Capture initial team scores", async () => {
            initialHomeTeamScore = await scorerPage.getTeamScores(scorerPage.homeTeamScore);
            initialAwayTeamScore = await scorerPage.getTeamScores(scorerPage.awayTeamScore);
        });

        await step("Add score for home team and validate score update", async () => {
            await scorerPage.addTeamScore(scorerPage.addHomeTeamScore);
            homeTeamScoreAfter = await scorerPage.getTeamScores(scorerPage.homeTeamScore);
            expect(homeTeamScoreAfter).not.toEqual(initialHomeTeamScore);
        });

        await step("Add score for away team and validate score update", async () => {
            await scorerPage.addTeamScore(scorerPage.addAwayTeamScore);
            awayTeamScoreAfter = await scorerPage.getTeamScores(scorerPage.awayTeamScore);
        ...
        await step("Undo home team score and validate score reset", async () => {
            ...
            homeTeamScoreAfterUndo = await scorerPage.getTeamScores(scorerPage.homeTeamScore);
        ...
        await step("Undo away team score and validate score reset", async () => {
            ...
            awayTeamScoreAfterUndo = await scorerPage.getTeamScores(scorerPage.awayTeamScore);
